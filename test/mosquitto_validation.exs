...
defmodule AuthHandler do
  ...
  @impl true
  def init(opts), do: %{sub_table: Keyword.fetch!(opts, :sub_table), event_table: Keyword.fetch!(opts, :event_table)}
...
defmodule Validation do
  ...
  def run do
    ...
    {:ok, tcp_pid} = MqttX.Server.start_link(PubSubHandler, [sub_table: sub_table, event_table: event_table],
      transport: MqttX.Transport.ThousandIsland, port: tcp_port)
    {:ok, ws_pid} = MqttX.Server.start_link(PubSubHandler, [sub_table: sub_table, event_table: event_table],
      transport: MqttX.Transport.WebSocket, port: ws_port)
    {:ok, auth_pid} = MqttX.Server.start_link(AuthHandler, [sub_table: sub_table, event_table: event_table],
      transport: MqttX.Transport.ThousandIsland, port: auth_port)
    Process.sleep(300)
  ...
  defp run_transport_suite(label, port, ws_flags) do
    ...
    r1 = [
      ...
      test("#{label}: connect and publish QoS 0", fn ->
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/basic/q0", "-m", "q0-msg", "-q", "0"])
      end),

      test("#{label}: connect and publish QoS 1", fn ->
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/basic/q1", "-m", "q1-msg", "-q", "1"])
      end),

      test("#{label}: connect and publish QoS 2", fn ->
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/basic/q2", "-m", "q2-msg", "-q", "2"])
      end),

      test("#{label}: publish with explicit client ID", fn ->
        run_cmd("mosquitto_pub", base_pub ++ ["-i", "#{label}-explicit-id", "-t", "#{label}/clientid", "-m", "id-test"])
      end),
      ...
      test("#{label}: publish large payload (256KB)", fn ->
        payload = String.duplicate("Y", 262144)
    ...
    r2 = [
      ...
      test("#{label}: pub/sub large payload round-trip (64KB)", fn ->
        ...
        pubsub_roundtrip_raw(base_sub, base_pub, "#{label}/ps/large64k", payload, 0, 0, fn output ->
    ...
    r3 = [
      ...
      test("#{label}: multiple subscriptions on same connection", fn ->
        sub_task = async_sub(base_sub ++ ["-t", "#{label}/multi/topicA", "-t", "#{label}/multi/topicB", "-C", "2", "-W", "5"])
        Process.sleep(500)
    ...
    r4 = [
      test("#{label}: retained message delivered on subscribe", fn ->
        ...
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/retained/test1", "-m", "retained-payload", "-r"])
        Process.sleep(200)
      ...
      test("#{label}: retained message overwrite", fn ->
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/retained/overwrite", "-m", "old-value", "-r"])
        Process.sleep(100)
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/retained/overwrite", "-m", "new-value", "-r"])
        Process.sleep(200)
        sub_task = async_sub(base_sub ++ ["-t", "#{label}/retained/overwrite", "-C", "1", "-W", "3"])
        expect_exact(sub_task, "new-value")
      end),

      test("#{label}: retained message deleted by empty publish", fn ->
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/retained/delete", "-m", "to-delete", "-r"])
        Process.sleep(100)
      ...
      test("#{label}: retained message with wildcard subscribe", fn ->
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/retained/wild/a", "-m", "rw-a", "-r"])
        Process.sleep(100)
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/retained/wild/b", "-m", "rw-b", "-r"])
        Process.sleep(200)
    ...
    r5 = [
      ...
      test("#{label}: will message NOT sent on graceful disconnect", fn ->
        ...
        run_cmd("mosquitto_pub", base_pub ++ [
          "-t", "#{label}/will/grace/trigger", "-m", "trigger",
          "--will-topic", "#{label}/will/grace/should_not_fire",
          "--will-payload", "ghost",
          "--will-qos", "0"
        ])
        Process.sleep(300)
        sub_task = async_sub(base_sub ++ ["-t", "#{label}/will/grace/should_not_fire", "-C", "1", "-W", "2"])
        expect_timeout(sub_task)
      end),

      test("#{label}: will message with QoS 1", fn ->
        sub_task = async_sub(base_sub ++ ["-t", "#{label}/will/q1", "-q", "1", "-C", "1", "-W", "5"])
        Process.sleep(500)
        raw_connect_with_will_then_crash(port, ws_flags,
    ...
    r6 = [
      test("#{label}: unsubscribe stops message delivery", fn ->
        ...
        sub_task = async_sub(base_sub ++ [
          "-t", "#{label}/unsub/keep", "-t", "#{label}/unsub/remove",
          "-U", "#{label}/unsub/remove",
          "-C", "1", "-W", "5"
        ])
        Process.sleep(500)
        # Publish to the removed topic first
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/unsub/remove", "-m", "should-not-get"])
        Process.sleep(200)
    ...
    r8 = [
      test("#{label}: binary-like payload (null bytes in string)", fn ->
        ...
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/payload/special", "-m", "line1\nline2\ttab"])
      end),

      test("#{label}: max-length client ID (23 chars MQTT 3.1.1)", fn ->
        run_cmd("mosquitto_pub", base_pub ++ ["-i", "12345678901234567890123", "-t", "#{label}/longid", "-m", "ok"])
      end),

      test("#{label}: very long client ID (128 chars)", fn ->
        long_id = String.duplicate("a", 128)
        run_cmd("mosquitto_pub", base_pub ++ ["-i", long_id, "-t", "#{label}/verylongid", "-m", "ok"])
      end),
    ...
    r9 = [
      test("#{label}: rapid connect/disconnect (10x)", fn ->
        ...
        if Enum.all?(results, &(&1 == :ok)), do: :ok, else: {:error, "Some rapid connections failed"}
      end),

      test("#{label}: concurrent publishers (5 simultaneous)", fn ->
        tasks = for i <- 1..5 do
          Task.async(fn ->
            run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/concurrent/#{i}", "-m", "c#{i}"])
          end)
        end
        results = Enum.map(tasks, &Task.await(&1, 10_000))
        if Enum.all?(results, &(&1 == :ok)), do: :ok, else: {:error, "Some concurrent publishes failed"}
      end),

      test("#{label}: concurrent subscribers receive same message", fn ->
        # Start 3 subscribers
        sub_tasks = for _i <- 1..3 do
          async_sub(base_sub ++ ["-t", "#{label}/concurrent/fanout", "-C", "1", "-W", "5"])
        end
        Process.sleep(500)
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/concurrent/fanout", "-m", "broadcast"])
        results = Enum.map(sub_tasks, fn task ->
      ...
      test("#{label}: publish with repeat (3x)", fn ->
        ...
        Process.sleep(500)
        run_cmd("mosquitto_pub", base_pub ++ ["-t", "#{label}/repeat", "-m", "rep",
          "--repeat", "3", "--repeat-delay", "0"])
  ...
  defp run_auth_suite(port) do
    ...
    results = [
      test("AUTH: valid credentials accepted", fn ->
        run_cmd("mosquitto_pub", base ++ ["-u", "valid_user", "-P", "valid_pass",
          "-t", "auth/ok", "-m", "authenticated"])
      end),

      test("AUTH: invalid password rejected", fn ->
        result = run_cmd("mosquitto_pub", base ++ ["-u", "valid_user", "-P", "wrong_pass",
          "-t", "auth/fail1", "-m", "should-fail"])
        case result do
          {:error, _} -> :ok
      ...
      test("AUTH: invalid username rejected", fn ->
        result = run_cmd("mosquitto_pub", base ++ ["-u", "wrong_user", "-P", "valid_pass",
          "-t", "auth/fail2", "-m", "should-fail"])
        case result do
          {:error, _} -> :ok
          :ok -> {:error, "Expected connection rejection but got success"}
        end
  ...
  defp run_cross_transport_suite(tcp_port, ws_port) do
    IO.puts("--- CROSS: TCP publisher → WS subscriber (same broker not possible, separate validation) ---\n")
    ...
    results = [
      test("CROSS: TCP and WS handle same topic identically", fn ->
        # Publish on TCP
        :ok = run_cmd("mosquitto_pub", ["-h", "127.0.0.1", "-p", "#{tcp_port}",
          "-t", "cross/same", "-m", "via-tcp", "-r"])
        Process.sleep(200)
        # Subscribe on TCP — should get retained
        sub1 = async_sub(["-h", "127.0.0.1", "-p", "#{tcp_port}",
          "-t", "cross/same", "-C", "1", "-W", "3"])
        r1 = expect_exact(sub1, "via-tcp")

        # Publish on WS
        :ok = run_cmd("mosquitto_pub", ["--ws", "-h", "127.0.0.1", "-p", "#{ws_port}",
          "-t", "cross/same", "-m", "via-ws", "-r"])
        Process.sleep(200)
        # Subscribe on WS — should get retained
        sub2 = async_sub(["--ws", "-h", "127.0.0.1", "-p", "#{ws_port}",
          "-t", "cross/same", "-C", "1", "-W", "3"])
        r2 = expect_exact(sub2, "via-ws")

        if r1 == :ok and r2 == :ok, do: :ok, else: {:error, "TCP: #{inspect(r1)}, WS: #{inspect(r2)}"}
      end),
    ]
  ...
  defp run_protocol_version_suite(port) do
    ...
    results = [
      ...
      test("PROTO: MQTT 3.1.1 (default)", fn ->
        run_cmd("mosquitto_pub", base ++ ["-V", "mqttv311", "-t", "proto/v311", "-m", "v311-test"])
      end),
      ...
      test("PROTO: MQTT 5.0 with user property", fn ->
        run_cmd("mosquitto_pub", base ++ ["-V", "5", "-t", "proto/v5/props",
          "-m", "props-test", "-D", "publish", "user-property", "key1", "value1"])
      end),

      test("PROTO: MQTT 5.0 with content type", fn ->
        run_cmd("mosquitto_pub", base ++ ["-V", "5", "-t", "proto/v5/content",
          "-m", ~s({"data": true}), "-D", "publish", "content-type", "application/json"])
      end),

      test("PROTO: MQTT 5.0 with message expiry", fn ->
        run_cmd("mosquitto_pub", base ++ ["-V", "5", "-t", "proto/v5/expiry",
          "-m", "expiring", "-D", "publish", "message-expiry-interval", "60"])
      end),

      test("PROTO: MQTT 5.0 with response topic and correlation data", fn ->
        run_cmd("mosquitto_pub", base ++ ["-V", "5", "-t", "proto/v5/reqresp",
          "-m", "request",
          "-D", "publish", "response-topic", "proto/v5/response",
          "-D", "publish", "correlation-data", "req-123"])
      end),

      test("PROTO: MQTT 5.0 session expiry", fn ->
        run_cmd("mosquitto_pub", base ++ ["-V", "5", "-t", "proto/v5/sessexp",
          "-m", "session-test", "-x", "60"])
      end),

      test("PROTO: MQTT 5.0 clean start + session expiry 0", fn ->
        run_cmd("mosquitto_pub", base ++ ["-V", "5", "-t", "proto/v5/cleanstart",
          "-m", "clean-test", "-x", "0"])
      end),

      test("PROTO: MQTT 5.0 topic alias", fn ->
        run_cmd("mosquitto_pub", base ++ ["-V", "5", "-t", "proto/v5/ta", "-m", "ta-msg",
  ...
  defp expect_exact(task, expected) do
    ...
    if exit_code == 0 and String.trim(output) == expected do
      ...
      {:error, "Expected #{inspect(expected)}, got #{inspect(String.trim(output))} (exit: #{exit_code})"}
    end
  end
  ...
  defp expect_lines(task, count) do
    ...
    if exit_code == 0 and length(lines) == count do
      ...
      {:error, "Expected #{count} lines, got #{length(lines)} (exit: #{exit_code}): #{inspect(Enum.take(lines, 5))}"}
    end
  end
  ...
  defp print_summary(results) do
    ...
    # Group by prefix
    groups = Enum.group_by(results, fn {name, _} ->
